...
public class CafePosSystem_pay extends Frame {
	...
	public Panel payLayout() throws Exception {
		this.removeAll();
		...
		Dialog dialog_pay = new Dialog(this, "결제 완료") {
			@Override
			public Insets getInsets() {
				return new Insets(40, 0, 15, 0);
			}
		...
		Dialog dialog_nomoney = new Dialog(this, "금액 부족") {
			@Override
			public Insets getInsets() {
				return new Insets(40, 0, 15, 0);
			}
		...
		tf_getmoney.addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {
				if (!(tf_getmoney.getText().equals("")) && e.getKeyCode() == KeyEvent.VK_ENTER) {
					pay(dialog_nomenu, dialog_pay, dialog_rankup, dialog_nomoney, f_title, f_subtitle, tf_usepoint,
							tf_gusno, tf_balance, tf_allmoney, tf_getmoney, tf_gusrank, tf_change, arr,
							hm_select_menu_count, hm_select_menu_price);
				}
	...
	// 결제 메서드
	public void pay(Dialog dialog_nomenu, Dialog dialog_pay, Dialog dialog_rankup, Dialog dialog_nomoney, Font f_title,
			Font f_subtitle, TextField tf_usepoint, TextField tf_gusno, TextField tf_balance, TextField tf_allmoney,
			TextField tf_getmoney, TextField tf_gusrank, TextField tf_change, ArrayList<Integer> arr,
			HashMap<Integer, Integer> hm_select_menu_count, HashMap<Integer, Integer> hm_select_menu_price) {
		try {
			int rankup = 0;
			String rankname = "";
			if (tf_allmoney.getText().equals("0")) {
				...
				dialog_nomoney.setSize(300, 150);
				dialog_nomoney.setLayout(new BorderLayout());

				Panel p_dialog_nomoney_center = new Panel(new GridLayout(4, 1));

				Label lb_dialog_nomoney_pop1 = new Label("주신 금액이 부족합니다.", Label.CENTER);
				Label lb_dialog_nomoney_pop2 = new Label("금액을 더 입력하세요.", Label.CENTER);
				Label lb_dialog_nomoney_pop3 = new Label();
				lb_dialog_nomoney_pop1.setFont(f_title);
				lb_dialog_nomoney_pop2.setFont(f_title);

				dialog_nomoney.add(p_dialog_nomoney_center, "Center");

				p_dialog_nomoney_center.add(lb_dialog_nomoney_pop3);
				p_dialog_nomoney_center.add(lb_dialog_nomoney_pop1);
				p_dialog_nomoney_center.add(lb_dialog_nomoney_pop2);
				lb_dialog_nomoney_pop3 = new Label();
				p_dialog_nomoney_center.add(lb_dialog_nomoney_pop3);

				Panel p_dialog_nomoney_south = new Panel(new FlowLayout());
				dialog_nomoney.add(p_dialog_nomoney_south, "South");
				Button bt_dialog_nomoney_popclose = new Button("확인");
				bt_dialog_nomoney_popclose.setPreferredSize(new Dimension(100, 20));
				bt_dialog_nomoney_popclose.setFont(f_subtitle);
				p_dialog_nomoney_south.add(bt_dialog_nomoney_popclose);
				dialog_nomoney.setVisible(true);
				dialog_nomoney.setLocationRelativeTo(p_main);
				bt_dialog_nomoney_popclose.addActionListener(new ActionListener() {

					@Override
					public void actionPerformed(ActionEvent e) {
						try {
							dialog_nomoney.setVisible(false);
							dialog_nomoney.dispose();
						} catch (Exception e1) {
							e1.printStackTrace();
						}
					}
